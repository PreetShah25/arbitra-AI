 ...
 * Steps 1–3 with robust Step-3 argument passing and strict booleans.
 */

/* ---------- UI atoms ---------- */
function Badge({ children, variant = "neutral" as "neutral" | "success" | "warning" | "danger" }) {
...
function Button({
  ...
  return (
    <button title={title} onClick={onClick} disabled={Boolean(disabled)} className={`px-3 py-2 rounded-xl text-sm ${cls} disabled:opacity-60 disabled:cursor-not-allowed transition`}>
      {children}
    </button>
  );
}
    ...
    <div className="space-y-2">
      {!videoUrl && (
        <div className="flex gap-2">
          <Button variant="primary" onClick={startRecording} disabled={recording}>Start recording</Button>
          <Button variant="default" onClick={stopRecording} disabled={!recording}>Stop & upload</Button>
        </div>
      )}
...
async function generateScreenshots(args: {
  ...
  if (!videoPreviewUrl || !currentTaskId) { alert("Please upload or record a video first."); return []; }
  const video = document.createElement("video");
  video.src = videoPreviewUrl; video.crossOrigin = "anonymous"; video.muted = true;
  ...
  for (let t = 0, i = 0; t < duration && i < 8; t += 2, i++) {
...
function draftUnderstanding({
  shots, draftDesc, company, currentTaskId, updateTask,
}: {
  ...
  if (currentTaskId) updateTask(currentTaskId, { understanding: { summary, questions, answers: ["", "", ""] } });
}
...
export default function App() {
  ...
  const onCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      } catch { alert("Failed to parse CSV. Expect header: period,AUR,DTC,Wholesale"); }
    };
  ...
  const downloadCsv = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `${selected}_KPIs.csv`; a.click(); URL.revokeObjectURL(url);
  };
  ...
  const updateTask = (id: string, patch: any) => {
    setTasks((prev) => ({ ...prev, [selected]: (prev[selected] || []).map((t) => (t.id === id ? { ...t, ...patch } : t)) }));
  };
  ...
  const deleteTask = (id: string, ticker?: string) => {
    ...
    if (currentTaskId === id) {
      setTaskModalOpen(false); setCurrentTaskId(null);
      setVideoPreviewUrl(""); setUploadProgress(0); setWizardStep(1);
  ...
  const openNewAnalysis = () => {
    setDraftName(""); setDraftDesc("");
    setVideoPreviewUrl(""); setUploadProgress(0);
    setWizardStep(1); setCurrentTaskId(null);
  ...
  const createDraftTask = () => {
    ...
    if (!desc) { alert("Please provide a non-empty description for the task."); return; }
    ...
    setCurrentTaskId(id); setWizardStep(2);
  };

  const onRecorded = (url: string) => {
    setVideoPreviewUrl(url); setUploadProgress(0);
    const int = setInterval(() => {
      setUploadProgress((p) => {
        const next = Math.min(100, p + 20);
        if (next >= 100) { clearInterval(int); if (currentTaskId) updateTask(currentTaskId, { status: "analyzing", videoUrl: url }); }
        return next;
      });
    }, 200);
  };

  const onFileUpload = (file?: File | null) => { if (!file) return; onRecorded(URL.createObjectURL(file)); };

  // —— Step 3 trigger that passes all required args:
  const goStep3 = async () => {
    await proceedToStep3({
      currentTask, videoPreviewUrl, currentTaskId,
      draftDesc, company, updateTask, setWizardStep,
    });
  };

  /* ---------- Render ---------- */
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(to bottom,#f8fafc,#ffffff)", color: "#111827", ["--accent" as any]: "#111827" }}>
              ...
              {watchlist.map((c) => (
                <div key={c.ticker} className={`flex items-center justify-between p-2 rounded-xl ${selected === c.ticker ? "bg-gray-50 border" : ""}`}>
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{c.sector} · AUR {c.kpi.AUR.toFixed(2)} · DTC {Math.round(c.kpi.DTC * 100)}%</div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="mt-2 flex gap-1">
                      <Button variant="ghost" onClick={() => setSelected(c.ticker)}>Open</Button>
                      <Button variant="ghost" title="Nudge price up" onClick={() => nudgePrice(c.ticker, +0.5)}>＋</Button>
                      <Button variant="ghost" title="Nudge price down" onClick={() => nudgePrice(c.ticker, -0.5)}>－</Button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {tab === "company" && (
            <>
              <Card
                ...
                actions={
                  <div className="flex items-center gap-3 text-sm text-gray-600">
                    <Button variant="primary" onClick={openNewAnalysis}>New Analysis</Button>
                    <label className="cursor-pointer">
                      Upload KPIs CSV <input type="file" accept=".csv" onChange={onCsvUpload} className="text-xs" />
                    </label>
                    <Button variant="ghost" onClick={downloadCsv}>Download sample CSV</Button>
                    <select className="border rounded-lg px-2 py-1" value={"line"} onChange={() => {}}>
              ...
              <Card title="Agent Tasks">
                <div className="space-y-2">
                  {(tasks[selected] || []).length === 0 && <div className="text-gray-400 text-sm">No tasks yet. Click <span className="font-medium">New Analysis</span> to create your first task.</div>}
                  {(tasks[selected] || []).map((t) => (
                    <div key={t.id} className="p-3 rounded-xl border flex items-center justify-between">
                      <div>
                        <div className="font-medium">{t.name} <span className="text-gray-400 text-xs">• {new Date(t.createdAt).toLocaleString()}</span></div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="destructive" onClick={() => deleteTask(t.id, t.ticker)}>Delete</Button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <Modal open={taskModalOpen} title={`New Analysis – ${company?.name} (${company?.ticker})`} onClose={() => setTaskModalOpen(false)}>
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button variant="primary" onClick={() => {
                ...
                setTasks((prev) => ({ ...prev, [selected]: [newTask, ...(prev[selected] || [])] }));
                setCurrentTaskId(id); setWizardStep(2);
              }} disabled={!canCreate}>Create draft</Button>
            </div>
          </div>
        )}
        ...
        {wizardStep === 2 && (
          <div className="space-y-3 text-sm">
            ...
            <RecordingWidget onRecorded={(url) => {
              ...
              const int = window.setInterval(() => {
                setUploadProgress((p) => {
                  ...
                  if (next >= 100) {
                    ...
                    if (currentTaskId) setTasks((prev) => ({
                      ...prev,
                      [selected]: (prev[selected] || []).map((t) => (t.id === currentTaskId ? { ...t, status: "analyzing", videoUrl: url } : t)),
                    }));
                  }
                  return next;
                });
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-600">
                ...
                <input type="file" accept="video/*" onChange={(e) => {
                  ...
                  // reuse handler:
                  const fake = { onRecorded: (u: string) => {} };
                  setVideoPreviewUrl(url);
              ...
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[var(--accent)]" style={{ width: `${uploadProgress}%` }} /></div>
            ...
            <div className="flex items-center justify-end pt-2 gap-2">
              ...
              <Button
                variant="primary"
                onClick={() => proceedToStep3({ currentTask, videoPreviewUrl, currentTaskId, draftDesc, company, updateTask, setWizardStep })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-72 overflow-auto border rounded-xl p-2">
                {(currentTask?.screenshots || []).length === 0 && <div className="col-span-2 text-xs text-gray-500">No screenshots yet. Use the extract button in Step 2.</div>}
                {(currentTask?.screenshots || []).map((s: any) => (
                  <div key={s.id} className="space-y-1">
                    ...
                    <input value={s.note || ""} onChange={(e) => {
                      const shots = (currentTask?.screenshots || []).map((x: any) => (x.id === s.id ? { ...x, note: e.target.value } : x));
                      if (currentTaskId) updateTask(currentTaskId, { screenshots: shots });
                    }} placeholder="Note (optional)" className="w-full border rounded-lg px-2 py-1 text-xs" />
            ...
            {/* Right: (Optional) Understanding placeholder */}
            <div>
              ...
              <div className="space-y-2 p-2 border rounded-xl mb-4">
                ...
                <textarea
                  value={currentTask?.understanding?.summary || ""}
                  onChange={(e) => { if (currentTaskId) updateTask(currentTaskId, { understanding: { ...(currentTask?.understanding || { questions: [], answers: [] }), summary: e.target.value } }); }}
              ...
              <div className="space-y-2 max-h-72 overflow-auto">
                {(currentTask?.plan || []).map((p: any) => (
                  <div key={p.id} className="p-2 border rounded-xl">
                    <div className="flex items-center gap-2">
                      <input value={p.title} onChange={(e) => { if (currentTaskId) updateTask(currentTaskId, { plan: (currentTask?.plan || []).map((x: any) => (x.id === p.id ? { ...x, title: e.target.value } : x)) }); }} className="flex-1 border rounded-lg px-2 py-1 text-sm" />
                      <label className="text-xs flex items-center gap-1">
                        <input type="checkbox" checked={Boolean(p.blocking)} onChange={(e) => { if (currentTaskId) updateTask(currentTaskId, { plan: (currentTask?.plan || []).map((x: any) => (x.id === p.id ? { ...x, blocking: e.target.checked } : x)) }); }} /> blocking
                      </label>
                    </div>
                    <textarea value={p.detail} onChange={(e) => { if (currentTaskId) updateTask(currentTaskId, { plan: (currentTask?.plan || []).map((x: any) => (x.id === p.id ? { ...x, detail: e.target.value } : x)) }); }} className="w-full border rounded-lg px-2 py-1 text-xs mt-2" placeholder="Details" />
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer className="max-w-6xl mx-auto px-4 py-10 text-xs text-gray-500">
        <div>Demo only — all data mocked. Ready for Step 4 (output) once this is live.</div>
      </footer>
    </div>
  );
}
